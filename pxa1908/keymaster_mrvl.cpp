...
static int marvell_keymaster_delete_all(const struct keymaster_device *device)
{
    if( teec_delete_all() )
        return -1;
...
static int marvell_keymaster_delete_keypair(const struct keymaster_device* dev,
...
static int marvell_keymaster_generate_keypair(const struct keymaster_device *dev,
...
static int marvell_keymaster_sign_data(const struct keymaster_device *dev,
...
static int marvell_keymaster_verify_data(const struct keymaster_device *dev,
        const void *params, const uint8_t *key_blob,
...
static int marvell_keymaster_get_keypair_public(const struct keymaster_device *dev,
...
static int marvell_keymaster_import_keypair(const keymaster_device_t* dev,
...
static int marvell_keymaster_open(const hw_module_t* module, const char* name,
        hw_device_t** device)
{
    keymaster_device * dev;

    if (strcmp(name, KEYSTORE_KEYMASTER) != 0)
        return -EINVAL;

    dev = new keymaster_device;
    if( dev == NULL )
        return -ENOMEM;
